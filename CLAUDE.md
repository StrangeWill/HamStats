...
- **HamStats.Data** — EF Core 8 + SQLite data layer. `HamStatsDbContext` and all entity models.
...
`Dockerfile` is a 3-stage build (Vue build → .NET publish → `aspnet:8.0` runtime) producing
...
`Kestrel__Endpoints__Http__Url=http://+:5000`. The container exposes TCP 5000 (HTTP) and UDP 16000
(N1MM), and stores the SQLite DB at `/data/hamstats.db` (mounted as a volume, like DireControl).
...
backend must be receiving N1MM UDP traffic on port 16000 to populate data.
